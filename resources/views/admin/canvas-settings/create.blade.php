...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Canvas API Instelling Toevoegen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.canvas-settings.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   name="apiurl" value="{{ old('apiurl', 'https://canvas.instructure.com') }}" type="url" required
                                   placeholder="https://canvas.instructure.com"/>
                        </label>
                        <p class="text-xs text-gray-500 mt-1">Voer de basis URL van de Canvas API in. Bijvoorbeeld: https://canvas.instructure.com</p>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   name="apitoken" value="{{ old('apitoken') }}" type="text" required
                                   placeholder="Voer hier je Canvas API token in"/>
                        </label>
                        <p class="text-xs text-gray-500 mt-1">Voer je persoonlijke Canvas API token in. Deze kun je genereren in je Canvas account instellingen.</p>
                    </div>

                    <div class="mb-4 flex items-center">
                        <label class="flex items-center text-sm">
                            <input type="checkbox" name="active" class="form-checkbox text-purple-600"
                                    {{ old('active') ? 'checked' : '' }}>
                            <span class="ml-2 text-gray-700">Actief</span>
                        </label>
                        <p class="text-xs text-gray-500 ml-4">Zet deze instelling op actief om de Canvas API te gebruiken.</p>
                    </div>
