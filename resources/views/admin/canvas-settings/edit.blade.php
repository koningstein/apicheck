...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Canvas API Instelling Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.canvas-settings.update', ['canvas_setting' => $setting->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   name="apiurl" value="{{ old('apiurl', $setting->apiurl) }}" type="url" required/>
                        </label>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   name="apitoken" type="text" placeholder="Laat leeg om huidige token te behouden"/>
                        </label>
                        <p class="text-xs text-gray-500 mt-1">Laat dit veld leeg als je het huidige token wilt behouden.</p>
                    </div>

                    <div class="mb-4 flex items-center">
                        <label class="flex items-center text-sm">
                            <input type="hidden" name="active" value="0">
                            <input type="checkbox" name="active" value="1" class="form-checkbox text-purple-600"
