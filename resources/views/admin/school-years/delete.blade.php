...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Schooljaar Verwijderen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.school-years.destroy', ['school_year' => $schoolYear->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   value="{{ $schoolYear->startdate ? date('Y-m-d', strtotime($schoolYear->startdate)) : '' }}" type="date" disabled/>
                        </label>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   value="{{ $schoolYear->enddate ? date('Y-m-d', strtotime($schoolYear->enddate)) : '' }}" type="date" disabled/>
                        </label>
                    </div>

                    <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4" role="alert">
                        <p class="font-bold">Let op!</p>
                        <p>Weet je zeker dat je dit schooljaar wilt verwijderen? Deze actie kan niet ongedaan worden gemaakt.</p>
                        <p>Als dit schooljaar aan klassen of andere gegevens is gekoppeld, kan het verwijderen gevolgen hebben voor die gegevens.</p>
                    </div>
