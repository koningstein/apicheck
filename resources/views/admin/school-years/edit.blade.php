...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Schooljaar Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.school-years.update', ['school_year' => $schoolYear->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   name="startdate" value="{{ old('startdate', $schoolYear->startdate ? date('Y-m-d', strtotime($schoolYear->startdate)) : '') }}" type="date" required/>
                        </label>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm">
                            ...
                            <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                            focus:outline-none focus:shadow-outline-purple form-input"
                                   name="enddate" value="{{ old('enddate', $schoolYear->enddate ? date('Y-m-d', strtotime($schoolYear->enddate)) : '') }}" type="date" required/>
                        </label>
                    </div>
